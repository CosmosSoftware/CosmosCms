    ...
    public class Utilities
    {
        ...
        public HomeController GetHomeController(ClaimsPrincipal user, bool allowSetup, IOptions<CosmosConfig>? siteOptions = null)
        {
            ...
            var controller = new HomeController(logger,
                siteOptions,
                dbContext,
                GetArticleEditLogic(dbContext), GetUserManager(), GetSignInManager()
            )
            {
                ControllerContext = { HttpContext = GetMockContext(user) }
            };
        ...
        {
            var options = new SendGridEmailProviderOptions(_cosmosOptions.Value.SendGridConfig.SendGridKey,
                _cosmosOptions.Value.SendGridConfig.EmailFrom);
            var emailSender = new SendGridEmailSender(Options.Create(options), GetLogger<SendGridEmailSender>());
            return emailSender;
        }
        ...
        public SignInManager<IdentityUser> GetSignInManager(ApplicationDbContext? dbContext = null)
        {
            if (dbContext == null)
            {
                dbContext = GetApplicationDbContext();
            ...
            var userStore = new UserStore<IdentityUser>(dbContext);
            var userManager = new SignInManager<IdentityUser>(GetUserManager(), null,
